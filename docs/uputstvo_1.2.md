...
**Verzija:** 1.2 (posle probne anotacije i radnog lista za pitanja)
...
**Ne pravimo spisak reči.** Pravilo je o kategoriji, ne o pojedinačnim rečima.
Spisak navodi anotatora da traži članove spiska umesto da se pita da li rečenica
ocenjuje. Primeri iznad su ilustracija, ne lista za proveru.
...
> „Njegovo telo su potom, **navodno**, uništili ili sakrili." → **OBJ**.
> `navodno` je ograda nad činjeničnom tvrdnjom. Rečenica ne sadrži ocenu, pa
> ostaje objektivna. (U probnoj anotaciji je ovo pogrešno označeno kao SUBJ,
> pozivanjem na P9 — a u rečenici nema pitanja.)
...
**Prvo proveri da li je rečenica subjektivna iz nekog drugog razloga.** Ako bi
kao tvrdnja bila `SUBJ` po P1, P4, P5, P10 ili P11, oblik pitanja ništa ne menja
i P9 se uopšte ne primenjuje.

> „Kakav je upravljački kapacitet šefa države i cijelog **Ćacilenda**?" → **SUBJ**
> po P1 (`Ćaciland` je ocena), a ne zato što je pitanje.

> „Zašto vandalizovane govornice još **ruže** gradove?" → **SUBJ** po P1.

Tek ako bi rečenica kao tvrdnja bila objektivna, primeni test:

**Može li se odgovor koji autor podrazumeva dati jednom rečju?**

| | |
|---|---|
| Da (`Ne.`, `Niko.`, `Nijedan.`) — retoričko pitanje | `SUBJ` |
| Ne — pitanje ostaje otvoreno, strukturira tekst | `OBJ` |

> „Da li mi **zaista** pričamo o dve nepomirljive ideologije?" → odgovor: „Ne." →
> **SUBJ**

> „Kako je sve ovo previdio političar sa toliko iskustva poput Dodika?" →
> odgovor: „Nije previdio." → **SUBJ**

> „Kako pomiriti istorijsko pamćenje sa političkom odgovornošću?" → nema odgovora
> u jednoj reči, pitanje ostaje otvoreno → **OBJ**

> „Šta će razlikovati naredne izbore od svih ranijih?" → **OBJ**
...
### P13 — Izmišljen navod → SUBJ

Navodnici znače tuđi iskaz (P6) samo kada iskaz pripada **prepoznatljivom
govorniku koji ga je stvarno izrekao**. Ako autor sam sastavi rečenicu i stavi
je u usta neimenovanoj grupi, da bi sažeo ili ismejao neki stav, to je autorov
stav i oznaka je `SUBJ`.

> „Ili, ukratko: 'Mi napadamo Nepalce, Filipince i Indijce jer su se naselili
> ovdje i uzimaju nam posao.'" → **SUBJ**

Provera: postoji li govornik kome se iskaz može pripisati? Ako je odgovor „niko
konkretno", navodnici su retorički, ne dokumentarni.

### P14 — Autorovo sopstveno sećanje

Prvo lice u kojem autor iznosi šta mu se dogodilo je izveštaj o činjenici i sam
po sebi je `OBJ`. Ako uz sećanje ide i ocena, odlučuje ocena.

> „Srela sam ga dok je čekao trolu." → **OBJ**
> „**Najveći štos** novca kao brucoš video sam kod jednog od te gospode." →
> **SUBJ**, zbog ocene `najveći štos`, ne zbog prvog lica.

Isti odnos kao u P5: autorova osećanja i doživljaji su činjenice o autoru;
ocena teme je nešto drugo.
...
| Pitanje: odgovor autora staje u jednu reč | **SUBJ** |
| Pitanje koje ostaje otvoreno | **OBJ** |
| Ironija, sarkazam | **SUBJ** |
| Izmišljen navod bez govornika | **SUBJ** |
| Autorovo sećanje bez ocene | **OBJ** |
| Poziv na delanje (*trebalo bi, mora se*) | **SUBJ** |
| Autorova sopstvena osećanja kao sadržaj | **OBJ** |
| Nadimak, titula, poslovica | **OBJ** |

**Kod pitanja prvo proveri ostala pravila** — ako je rečenica subjektivna kao tvrdnja, pitanje ništa ne menja.
